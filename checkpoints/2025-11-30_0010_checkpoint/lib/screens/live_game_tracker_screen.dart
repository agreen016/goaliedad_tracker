...
class _LiveGameTrackerScreenState extends State<LiveGameTrackerScreen> {
  ...
  void _rebuildMarkersFromEvents() {
    try {
      ...
      debugPrint(
        '_rebuildMarkersFromEvents: loaded ${events.length} events for game=${widget.game.id}',
      );
      ...
      debugPrint(
        '_rebuildMarkersFromEvents: in-memory markers before merge count=${markers.length} ids=[${markers.map((m) => m.eventId).join(',')}]',
      );
      debugPrint(
        '_rebuildMarkersFromEvents: persisted event ids=[${persistedById.keys.join(',')}]',
      );
    ...
    switch (action) {
        ...
        showDialog(
          ...
          if (pickedPlayer != null) {
            setState(() {
              ...
              debugPrint(
                'Assigned player ${pickedPlayer.number} to position $position',
              );
            });
          }
        });
      ...
      case 'clear_player':
        setState(() {
          ...
          debugPrint('Cleared player from $position');
        });
        break;
      ...
      case 'clear_line':
        setState(() {
          ...
          debugPrint('Line cleared — Goalie preserved');
        });
        break;
        ...
        showDialog(
          ...
          if (pickedGoalie != null) {
            setState(() {
              ...
              debugPrint('Goalie changed to ${pickedGoalie.name}');
            });
          }
        });
        break;
      ...
      case 'pull_goalie':
        setState(() {
          ...
          debugPrint('Goalie pulled — Empty Net');
        });
        break;
