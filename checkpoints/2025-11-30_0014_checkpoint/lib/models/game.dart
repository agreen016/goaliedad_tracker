...
class Game extends HiveObject {
  ...
  void addEvent(GameEvent event) {
    ...
    try {
      // Log event persistence to help debug marker races where persisted
      // coordinates differ from the in-memory UI. Also log the identity hash
      // of the details Map so we can detect accidental reuse of the same Map
      // instance across multiple events.
      // ignore: avoid_print
      print(
        'Game.addEvent: adding event id=${copiedEvent.id} type=${copiedEvent.type} details=${copiedEvent.details} detailsHash=${copiedEvent.details.hashCode}',
      );
    } catch (_) {}
