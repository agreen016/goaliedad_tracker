...
class _TeamHubScreenState extends State<TeamHubScreen> {
  ...
  List<Map<String, dynamic>> _getGoalieRows() {
    ...
    for (final g in filteredGames) {
      ...
      final goals = <Goal>[];
      try {
        final boxG = Hive.box<Goal>('goals');
        goals.addAll(boxG.values.where((gg) => gg.gameId == g.id));
      } catch (_) {}

      for (final goal in goals) {
        final goalieId = goal.goalieId;
        if (goalieId.isNotEmpty && accum.containsKey(goalieId)) {
          // if goal.teamId != widget.team.id it's GA for this team's goalie
          if (goal.teamId != widget.team.id) {
      ...
      for (final ev in events) {
        if (ev.type == 'shot' || ev.type == 'goal') {
          ...
          // record that this goalie appeared in the game
          if (goalieId.isNotEmpty) goaliesSeen.add(goalieId);
          if (goalieId.isNotEmpty && accum.containsKey(goalieId)) {
            // Count SOG only when the shot was taken by the OPPOSING team
            if (ev.teamId != widget.team.id) {
              accum[goalieId]!['SOG'] = (accum[goalieId]!['SOG'] ?? 0) + 1;
            }
          }
          // GA is attributed from Goal records (handled earlier), so do not
          // double-count here by incrementing GA again for event-level goals.
        }
      }
    ...
    return Padding(
      ...
      child: ListView(
        children: [
          ...
          const SizedBox(height: 16),
          // Season heatmap controls
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 8.0),
            child: Row(
              children: [
                const Text('Goalie heatmap (season):'),
                const SizedBox(width: 8),
                Expanded(
                  child: DropdownButton<String>(
                    value: _selectedSeasonHeatmapGoalie,
                    hint: const Text('Select goalie'),
                    isExpanded: true,
                    items: _getGoalieRows().map((g) {
                    ...
                    onChanged: (val) =>
                        setState(() => _selectedSeasonHeatmapGoalie = val),
                  ),
                ),
              ],
            ),
          ...
          const SizedBox(height: 8),
          if (_selectedSeasonHeatmapGoalie != null)
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 8.0),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Season shot heatmap for selected goalie'),
                  const SizedBox(height: 8),
                  // Heatmap mode toggle + resolution slider
                  Row(
                    children: [
                      const Text('Mode:'),
                      const SizedBox(width: 8),
                      ToggleButtons(
                        isSelected: [
                          _seasonHeatmapMode == HeatmapMode.grid,
                          _seasonHeatmapMode == HeatmapMode.smooth,
                        ],
                        onPressed: (idx) => setState(() {
                          _seasonHeatmapMode = idx == 0
                              ? HeatmapMode.grid
                              : HeatmapMode.smooth;
                        }),
                        children: const [
                      ...
                      if (_seasonHeatmapMode == HeatmapMode.grid) ...[
                        const Text('Resolution'),
                        Expanded(
                          child: Slider(
                            value: _seasonHeatmapCols.toDouble(),
                            min: 8,
                            max: 120,
                            divisions: 14,
                            label: 'Cols ${_seasonHeatmapCols}',
                            onChanged: (v) =>
                                setState(() => _seasonHeatmapCols = v.toInt()),
                          ),
                        ),
                      ],
                    ],
                  ...
                  Builder(
                    builder: (ctx) {
                      final pts = _collectSeasonShotsForGoalie(
                        _selectedSeasonHeatmapGoalie!,
                      );
                      ...
                      return ShotHeatmap(
                        ...
                        points: pts,
                        color: accentColor,
                        mode: _seasonHeatmapMode,
                        gridCols: _seasonHeatmapCols,
                        gridRows: (_seasonHeatmapCols / 2)
                            .clamp(6, 200)
                            .toInt(),
                        onCellTap: (col, row, count, globalPos) {
                          final pct = total > 0
                              ? (count / total * 100).toStringAsFixed(1)
                              : '0.0';
                          final overlay = OverlayEntry(
                            builder: (ovCtx) {
                              // position slightly above the tap
                              final dx = globalPos.dx;
                              final dy = globalPos.dy - 40.0;
                              return Positioned(
                                left: dx.clamp(
                                  8.0,
                                ...
                                child: Material(
                                  ...
                                  child: Container(
                                    ...
                                    child: Text(
                                      'Cell ($col, $row): $count shots — $pct% of total',
...
Widget _buildTeamLeadersList(Color accentColor, Map<String, String> leaders) {
  ...
  final entries = leaders.entries.where((e) => e.value.isNotEmpty).toList();
